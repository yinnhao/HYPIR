...
inputs = [
    ...
    gr.Slider(minimum=1, maximum=8, value=1, label="Upscale Factor", step=1),
]

iface = gr.Interface(
    fn=process,
    inputs=inputs,
    outputs=[gr.Image(type="pil", format="png"), gr.Textbox(label="status")],
    title="Pixel Phoenix: Reviving Your Visual Memories",
)
iface.launch(server_name="0.0.0.0" if not args.local else "127.0.0.1", server_port=args.port)
